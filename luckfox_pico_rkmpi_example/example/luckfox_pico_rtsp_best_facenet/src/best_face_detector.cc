...
struct YoloFaceCandidate {
    float left, top, right, bottom, score;
};

// Anchors embedded by the standard Ultralytics YOLOv5 Detect head used by
// best_rknpu_rv1106_int8.rknn (three anchors for strides 8, 16 and 32).
static const int YOLO_ANCHORS[3][6] = {
    {10, 13, 16, 30, 33, 23},
    {30, 61, 62, 45, 59, 119},
    {116, 90, 156, 198, 373, 326}
};

static float box_iou(const YoloFaceCandidate& a, const YoloFaceCandidate& b)
{
    const float w = std::max(0.0f, std::min(a.right, b.right) -
                                    std::max(a.left, b.left));
    const float h = std::max(0.0f, std::min(a.bottom, b.bottom) -
                                    std::max(a.top, b.top));
    const float intersection = w * h;
    const float area_a = std::max(0.0f, a.right - a.left) *
                         std::max(0.0f, a.bottom - a.top);
    const float area_b = std::max(0.0f, b.right - b.left) *
                         std::max(0.0f, b.bottom - b.top);
    const float total = area_a + area_b - intersection;
    return total > 0.0f ? intersection / total : 0.0f;
}

static float detector_env_float(const char* name, float fallback,
...
static void set_box_landmark(object_detect_result* result, int index,
                             float x_ratio, float y_ratio)
{
    const float width = result->box.right - result->box.left;
    const float height = result->box.bottom - result->box.top;
    result->point[index].x = clamp(result->box.left + width * x_ratio, 0, 640);
    result->point[index].y = clamp(result->box.top + height * y_ratio, 0, 640);
}

static int inference_yolov5_face_model(rknn_app_context_t* app_ctx,
                                       object_detect_result_list* results)
{
    const float confidence_threshold = detector_env_float(
        "DETECT_CONF_THRESHOLD", 0.50f, 0.0001f, 1.0f);
    const float nms_threshold = detector_env_float(
        "DETECT_NMS_THRESHOLD", 0.20f, 0.01f, 1.0f);
    std::vector<YoloFaceCandidate> candidates;

    for (uint32_t branch = 0; branch < app_ctx->io_num.n_output; ++branch) {
        const rknn_tensor_attr& attr = app_ctx->output_attrs[branch];
        // RKNN_QUERY_NATIVE_NHWC_OUTPUT_ATTR reports [N,W,H,C] on RV1106.
        const int grid_w = attr.dims[1];
        const int grid_h = attr.dims[2];
        const int channels = attr.dims[3];
        if (branch >= 3 || channels != 18 || grid_w <= 0 || grid_h <= 0) {
            printf("[best-detector] unexpected output %u: %dx%dx%d\n",
                   branch, grid_w, grid_h, channels);
            return -1;
        }

        const int stride = app_ctx->model_width / grid_w;
        const int8_t* output = static_cast<const int8_t*>(
            app_ctx->output_mems[branch]->virt_addr);
        for (int y = 0; y < grid_h; ++y) {
            for (int x = 0; x < grid_w; ++x) {
                for (int anchor = 0; anchor < 3; ++anchor) {
                    const int offset = (y * grid_w + x) * channels + anchor * 6;
                    const int8_t* value = output + offset;
                    const float objectness = deqnt_affine_to_f32(
                        value[4], attr.zp, attr.scale);
                    const float class_score = deqnt_affine_to_f32(
                        value[5], attr.zp, attr.scale);
                    const float score = objectness * class_score;
                    if (score < confidence_threshold)
                        continue;

                    float center_x = deqnt_affine_to_f32(value[0], attr.zp,
                                                         attr.scale) * 2.0f - 0.5f;
                    float center_y = deqnt_affine_to_f32(value[1], attr.zp,
                                                         attr.scale) * 2.0f - 0.5f;
                    float width = deqnt_affine_to_f32(value[2], attr.zp,
                                                      attr.scale) * 2.0f;
                    float height = deqnt_affine_to_f32(value[3], attr.zp,
                                                       attr.scale) * 2.0f;
                    center_x = (center_x + x) * stride;
                    center_y = (center_y + y) * stride;
                    width = width * width * YOLO_ANCHORS[branch][anchor * 2];
                    height = height * height * YOLO_ANCHORS[branch][anchor * 2 + 1];

                    YoloFaceCandidate candidate;
                    candidate.left = center_x - width * 0.5f;
                    candidate.top = center_y - height * 0.5f;
                    candidate.right = center_x + width * 0.5f;
                    candidate.bottom = center_y + height * 0.5f;
    ...
    std::sort(candidates.begin(), candidates.end(),
              [](const YoloFaceCandidate& a, const YoloFaceCandidate& b) {
                  return a.score > b.score;
              });
    std::vector<YoloFaceCandidate> selected;
    for (const YoloFaceCandidate& candidate : candidates) {
        bool suppressed = false;
        for (const YoloFaceCandidate& kept : selected) {
            if (box_iou(candidate, kept) > nms_threshold) {
                suppressed = true;
                break;
            }
        }
    ...
    results->count = static_cast<int>(selected.size());
    for (int i = 0; i < results->count; ++i) {
        object_detect_result* result = &results->results[i];
        result->box.left = clamp(selected[i].left, 0, app_ctx->model_width);
        result->box.top = clamp(selected[i].top, 0, app_ctx->model_height);
        result->box.right = clamp(selected[i].right, 0, app_ctx->model_width);
        result->box.bottom = clamp(selected[i].bottom, 0, app_ctx->model_height);
        result->prop = selected[i].score;

        // This detector only returns boxes. Approximate the ArcFace landmark
        // layout so the unchanged FaceNet alignment pipeline can still run.
        set_box_landmark(result, 0, 0.32f, 0.38f); // left eye
        set_box_landmark(result, 1, 0.68f, 0.38f); // right eye
        set_box_landmark(result, 2, 0.50f, 0.57f); // nose
        set_box_landmark(result, 3, 0.38f, 0.75f); // left mouth
        set_box_landmark(result, 4, 0.62f, 0.75f); // right mouth
    }
    return 0;
}
...
{
    struct timespec bench_start{}, bench_run_done{}, bench_post_done{};
    static int bench_frame = 0;
    const bool bench_detector = getenv("BENCH_DETECTOR") != nullptr;
    if (bench_detector)
    ...
    if (bench_detector)
        clock_gettime(CLOCK_MONOTONIC, &bench_run_done);

    // best_rknpu_rv1106_int8.rknn is a one-class YOLOv5 model with three
    // [N,W,H,18] output branches. Keep the public detector API unchanged so
    // FaceNet, anti-spoof, attendance, MQTT and RTSP need no modifications.
    if (app_ctx->io_num.n_output == 3 &&
        app_ctx->output_attrs[0].dims[3] == 18) {
        ret = inference_yolov5_face_model(app_ctx, od_results);
        if (bench_detector) {
            clock_gettime(CLOCK_MONOTONIC, &bench_post_done);
            const long run_us =
                (bench_run_done.tv_sec - bench_start.tv_sec) * 1000000L +
                (bench_run_done.tv_nsec - bench_start.tv_nsec) / 1000L;
            const long post_us =
                (bench_post_done.tv_sec - bench_run_done.tv_sec) * 1000000L +
                (bench_post_done.tv_nsec - bench_run_done.tv_nsec) / 1000L;
            printf("[detector-bench] frame=%d run_us=%ld post_us=%ld total_us=%ld\n",
                   bench_frame++, run_us, post_us, run_us + post_us);
        }
        return ret;
    }
