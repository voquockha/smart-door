                  ...
                  const char *anti_spoof_model_path)
{
    ...
    if (!locker_ui_init()) {
        printf("[run] cannot initialize Smart Locker LCD UI\n");
        return -1;
    }
    ...
    const bool rtsp_enabled = env_bool("RTSP_ENABLED", false);
    ...
    printf("[run] Smart Locker ready (RTSP=%s)\n",
           rtsp_enabled ? "enabled on /live/0" : "disabled");
    ...
    while (!g_stop_requested) {
        ...
        if (s32Ret == RK_SUCCESS) {
            ...
            bgr.copyTo(frame);
            locker_ui_update_camera(bgr.data, bgr.cols, bgr.rows,
                                    (int)bgr.step);
            ...
            for (int i = 0; i < detected_face_count; i++) {
                ...
                /*
                 * Smart Locker identities are intentionally temporary and
                 * independent of the employee face database. Deposit stores
                 * an averaged live embedding; pickup matches only occupied
                 * locker embeddings and deletes the match after opening.
                 */
                locker_ui_process_embedding(
                    face_fp32, FACE_DB_EMBED_DIM, anti_result.real_score);
                strncpy(face_names[i], "TEMP FACE",
                        FACE_DB_NAME_LEN - 1);
                face_names[i][FACE_DB_NAME_LEN - 1] = '\0';
                face_confidences[i] = 1.0f;
                face_dists[i] = 0.0f;
                face_identity_confirmed[i] =
                    !locker_ui_is_scanning();
                face_instructions[i] = locker_ui_is_scanning()
                    ? "PROCESSING TEMP FACE" : "SMART LOCKER OK";
                continue;
                ...
                if (strcmp(face_names[i], "UNKNOWN") != 0) {
                    ...
                    face_instructions[i] = "SMART LOCKER OK";
                }
            }
